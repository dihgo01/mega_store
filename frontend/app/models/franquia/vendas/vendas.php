...
class QV_Produtos {
    ...
    public function consulta() {
        ...
        // VARIAVEIS
        $slug = $this->slug;
        $QV_idUnidade = $_SESSION['Authentication']['franquias'][$_SESSION['Authentication']['franquiaActive']]['unidade'];
        ...
        if($QV_idUnidade == 573) {
            $queryCustom = "P.preco_ecommerce AS PRECO,";
        } else {
            $queryCustom = "P.preco_sellout AS PRECO,";
        }        

        // CONSULTA DADOS PRODUTO
        $consultaProduto = mysqli_query($conexao, "
            SELECT P.id AS ID_PRODUTO, P.slug AS SLUG, P.nome_comercial AS NOME, ".$queryCustom." P.descricao AS DESCRICAO, C.nome AS CATEGORIA, C2.nome AS COR, M.nome AS MATERIAL, E.nome AS ESTILO, D.nome AS DETALHE, P.salto_tamanho_id AS SALTO_ID, S1.nome AS SALTO, S2.nome AS SALTO_ALTURA, C3.nome AS COLECAO
            FROM qv_produtos P
            INNER JOIN qv_categorias C ON C.id = P.categoria_id
            INNER JOIN qv_cores C2 ON C2.id = P.cor_id
            INNER JOIN qv_colecoes C3 ON C3.id = P.colecao_id
            LEFT JOIN qv_materiais M ON M.id = P.material_id
            LEFT JOIN qv_estilos E ON E.id = P.estilo_id
            LEFT JOIN qv_detalhes D ON D.id = P.detalhe_id
            LEFT JOIN qv_saltos_tamanhos S1 ON S1.id = P.salto_tamanho_id
            LEFT JOIN qv_saltos_alturas S2 ON S2.id = P.salto_altura_id
            WHERE P.slug = '".$slug."' ");
        ...
        // GRADE DO PRODUTO
        if($QV_idUnidade == 573) {
            $produto_grade = intranetEstoqueGrade($resProd['ID_PRODUTO']);
            $consultaProtheus = protheusEstoqueGrade($resProd['ID_PRODUTO']);
            $protheusEnderecos = "";
            for($i=0; $i < COUNT($consultaProtheus); $i++) {
                $protheusEnderecos .= (!empty($consultaProtheus[$i]['Endereco']) ? $consultaProtheus[$i]['Endereco']." / " : '');
            }
            $protheus = array('status' => true, 'conteudo' => protheusEstoqueGrade($resProd['ID_PRODUTO']), 'enderecos' => $protheusEnderecos);
        } else {
            $produto_grade = vitrineEstoqueGrade($resProd['ID_PRODUTO'],$QV_idUnidade);
            $protheus = array('status' => false);
        }     
        
        // ESPECIFICACOES PRODUTO
        $produtoEspecificacoes = array();
        if(!empty($resProd['COR'])) { 
            $produtoEspecificacoes[] = array('NOME'=> 'Cor', 'VALOR'=> $resProd['COR']); 
        }
        if(!empty($resProd['MATERIAL'])) { 
            $produtoEspecificacoes[] = array('NOME'=> 'Material', 'VALOR'=> $resProd['MATERIAL']);
        }
        if(!empty($resProd['ESTILO'])) { 
            $produtoEspecificacoes[] = array('NOME'=> 'Estilo', 'VALOR'=> $resProd['ESTILO']);
        }
        if(!empty($resProd['DETALHE'])) { 
            $produtoEspecificacoes[] = array('NOME'=> 'Detalhe', 'VALOR'=> $resProd['DETALHE']);
        }
        if(!empty($resProd['SALTO'])) { 
            $produtoEspecificacoes[] = array('NOME'=> 'Salto', 'VALOR'=> $resProd['SALTO']);
        }
        if(!empty($resProd['SALTO_ALTURA'])) { 
            $produtoEspecificacoes[] = array('NOME'=> 'Altura do Salto (cm)', 'VALOR'=> $resProd['SALTO_ALTURA']);
        }
        
        // ARRAY DADOS DO PRODUTO
        $produtoInfo = array('ID' => $resProd['ID_PRODUTO'], 'NOME' => $resProd['NOME'], 'SLUG' => $resProd['SLUG'], 'PRECO' => $resProd['PRECO'], 'DESCRICAO' => $resProd['DESCRICAO'], 'CATEGORIA' => $resProd['CATEGORIA'], 'COR' => $resProd['COR'], 'COLECAO' => $resProd['COLECAO'], 'FOTOS' => $produto_foto, 'GRADE' => $produto_grade, 'ESPECIFICACOES' => $produtoEspecificacoes, 'PROTHEUS' => $protheus);

        // VALIDACAO FINAL                 
        if($consultaProduto) {
            $resultadoFinal = array('resultado'=>true, 'mensagem'=>'Consulta Realizada com Sucesso', 'conteudo'=>$produtoInfo);
        } else {
            $resultadoFinal = array('resultado'=>false, 'mensagem'=>'Erro ao Realizar Consulta. Contate o Suporte!');
        }        
...
class QV_Estoque {
    ...
    function __construct($idUnidade,$filtros){
        $this->paginacao    = $filtros['F_paginacao'];
        $this->categoria    = $filtros['F_categoria'];
        $this->buscador     = $filtros['F_buscador'];
        $this->tamanho      = $filtros['F_tamanho'];
        $this->salto        = $filtros['F_salto'];
        $this->ordenacao    = $filtros['F_order'];
    }    
    ...
    public function consulta() {
        ...
        $QV_categoria   = $this->categoria;
        $QV_tamanho     = $this->tamanho;
        $QV_salto       = $this->salto;
        $QV_buscador    = $this->buscador;
        $QV_paginacao   = $this->paginacao;
        $QV_ordenacao   = $this->ordenacao;

        // ARMAZENA RESULTADOS
        $resConteudo = array();

        // VALIDANDO QUERY SALTO
        if($QV_salto == 'Sem_Salto') {
            $querySalto = " AND P.salto_tamanho_id = 1 ";
        } elseif($QV_salto == 'Com_Salto') {
            $querySalto = " AND P.salto_tamanho_id IN(2,3,4,5) ";
        } else {
            $querySalto = "";
        }

        // QUERY ORDENACAO
        if($QV_ordenacao == 'cadastro') {
            $queryOrder = " ID_PRODUTO DESC ";
        } else {
            $queryOrder = " NOME ASC ";
        }

        // VALIDANDO UNIDADE PARA CONSULTA DO ESTOQUE - INTRANET ou VITRINE
        if($QV_idUnidade == 573) {
           
            // CONSULTANDO ESTOQUE NA INTRANET LOJA QV e OUTLET
            $consulta = mysqli_query($conexao, "SELECT PE.produto_id AS ID_PRODUTO, 
            (COALESCE(sum(CASE WHEN ( PE.tipo = 'entrada' ) THEN PE.quantidade ELSE 0 END),0) - COALESCE(sum(CASE WHEN ( PE.tipo = 'saida' ) THEN PE.quantidade ELSE 0 END),0)) AS SALDO_ATUAL, P.nome_comercial AS NOME, P.slug AS SLUG, C.nome AS CATEGORIA, PE.preco_ecommerce AS PRECO, CONCAT(LPAD(MONTH(P.created_at), 2, '0'),'/',YEAR(P.created_at)) AS PERIODO_CRIACAO 
            ...
            LEFT JOIN qv_saltos_tamanhos ST ON ST.id = P.salto_tamanho_id
            WHERE PE.loja_id IN(1,137) AND PE.ativado = 1 AND PE.deleted_at IS NULL AND P.ativado = 1 AND P.ecommerce = 1 
    
            ".($QV_categoria > 0 ? ' AND P.categoria_id = '.$QV_categoria : '')." 
            ".($QV_tamanho > 0 ? ' AND G.codigo = \''.$QV_tamanho.'\'' : '')."
            ".($QV_buscador != '' ? ' AND(P.nome_comercial LIKE "%'.$QV_buscador.'%" OR P.codigo LIKE "%'.$QV_buscador.'%")' : '')." 
            ".$querySalto."
    
            GROUP BY ID_PRODUTO 
            HAVING SALDO_ATUAL > 0 
            ORDER BY ".$queryOrder." 
            LIMIT ".($QV_paginacao == 0 ? '0,9' : ($QV_paginacao).',9' )." ");

        } else {

            // CONSULTANDO ESTOQUE VITRINE
            $consulta = mysqli_query($conexao, "SELECT E.idProduto AS ID_PRODUTO, 
            COALESCE(sum(E.quantidade),0) AS SALDO_ATUAL, E.tamanho AS GRADE, P.nome_comercial AS NOME, P.slug AS SLUG, C.nome AS CATEGORIA, P.preco_sellout AS PRECO, CONCAT(LPAD(MONTH(P.created_at), 2, '0'),'/',YEAR(P.created_at)) AS PERIODO_CRIACAO 
            FROM PDV_estoque E
            INNER JOIN qv_produtos P ON P.id = E.idProduto
            INNER JOIN qv_categorias C ON C.id = P.categoria_id
            LEFT JOIN qv_saltos_tamanhos ST ON ST.id = P.salto_tamanho_id        
            WHERE E.idUnidade = ".$QV_idUnidade." 
    
            ".($QV_categoria > 0 ? ' AND E.idCategoria = '.$QV_categoria : '')." 
            ".($QV_tamanho > 0 ? ' AND E.tamanho = \''.$QV_tamanho.'\'' : '')." 
            ".($QV_buscador != '' ? ' AND(P.nome_comercial LIKE "%'.$QV_buscador.'%" OR P.codigo LIKE "'.$QV_buscador.'%")' : '')."
            ".$querySalto."
        ...
        if(mysqli_num_rows($consulta) > 0) {
            WHILE($resultado = mysqli_fetch_array($consulta)) {
                ...
                // GERANDO IMAGEM
                $produto_foto = qvImagens_Intranet($resultado['ID_PRODUTO']);

                // CHECANDO PRODUTO NOVIDADE
                $PERIODO_ATUAL = DATE("m/Y");
                if($resultado['PERIODO_CRIACAO'] == $PERIODO_ATUAL) {
                    $tagProdutoNovo = true;
                } else {
                    $tagProdutoNovo = false;
                }
                ...
                $resConteudo['ITENS'][] = array(
                    ...
                    'PRECO'         => numeroDecimal($resultado['PRECO'],2),
                    'FOTO'          => $produto_foto[0],
                    'NOVIDADE'      => $tagProdutoNovo
                );
            }
        }
...
class QV_Clientes {
    ...
    public function consulta() {
        ...
        $consulta = mysqli_query($conexao, "SELECT *
                                FROM PDV_clientes
                                WHERE cpf = '".limpar($QV_cpf)."' AND idUnidade = ".$QV_idUnidade." AND dataDelete IS NULL");
        $resConteudo['RESULTADOS'] = mysqli_num_rows($consulta);   
        if(mysqli_num_rows($consulta) > 0) {
            WHILE($resultado = mysqli_fetch_array($consulta)) {
