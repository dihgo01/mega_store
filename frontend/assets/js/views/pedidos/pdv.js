var prodCategorias_consulta = function() {
	return {
		init: function() {
			...
			var table = $('#responsive-datatable').DataTable({
				...
				"stateSave": false,
				"ajax": {
					"url": sessionStorage.getItem("dominio")+"/app/models/produtos/DT_categorias_consultar.php",
					"data": function (d) {
				...
				"language": {
					"url": sessionStorage.getItem("dominio")+"/inc/datatables/json/Portuguese-Brasil.json"
				},
			...
			// APAGAR CATEGORIA
			$('body').on('click', 'a.categoriaDelete', function() {
				...
				Swal.fire({
					...
					// CARREGA O TOASTR
					toastr["success"]("Enviando sua Solicitação", "Produtos / Categorias");					
					...
					if(result.value) {
						...
						$.ajax({
							...
							dataType : "json",
							data: 	'idItem='+idItem,
							url: '/app/models/produtos/categorias.php',
						...
						Swal.fire('Apagar Categoria', 'Operação foi Cancelada.', 'info');
					}
...
var prodCategorias_novo = function() {
	return {
		init: function(formID) {
			...
			$(formID).submit(function(e) {
				...
				var destino = '/app/models/produtos/categorias.php';
					...
					enviarForm(formID,destino, function(retorno) {
						...
						if(retorno.resultado === true) {
							...
							Swal.fire({
								...
								if(result.value) {
									// REDIRECT
									window.location = sessionStorage.getItem("dominio")+'/produtos/categorias/list';
								}
							});
